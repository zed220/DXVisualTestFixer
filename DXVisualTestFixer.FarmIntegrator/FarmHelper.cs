...
namespace DXVisualTestFixer.Farm {
        ...
        List<IFarmTaskInfo> IFarmIntegrator.GetAllTasks(Repository[] repositories) {
            ...
            foreach(var repository in repositories) {
                if(Repository.IsNewVersion(repository.Version)) {
                    var success = GetTaskStatus(repository.GetTaskName_New()).BuildStatus == FarmIntegrationStatus.Success;
                    result.Add(new FarmTaskInfo(repository, GetTaskUrl(repository.GetTaskName_New())) { Success = success });
                    continue;
                }
    ...
    public class FarmHelper {
        ...
        public string GetTaskUrl(string task) {
            lock(this.syncLocker) {
                return CalcTaskUrl(task);
            }
        }
        ...
        string CalcTaskUrl(string task) {
            ProjectTagI tag = FindTask(task);
            if(tag == null)
                return null;
            return tag.weburl;
